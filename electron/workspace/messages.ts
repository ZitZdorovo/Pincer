...
function inlineBase64(value: unknown, mimeType: string): string {
  ...
  if (!raw) return '';
  const dataUrl = raw.match(/^data:([^;,]+);base64,([A-Za-z0-9+/]*={0,2})$/i);
  if (dataUrl) return !mimeType || dataUrl[1].toLowerCase() === mimeType.toLowerCase() ? dataUrl[2] : '';
  return raw;
}
