...
const api: PincerApi = {
  settings: {
    ...
    saveMany: (entries) => ipcRenderer.invoke('pincer:settings:saveMany', entries),
  },
  secrets: {
    list: () => ipcRenderer.invoke('pincer:secrets:list'),
    set: (input) => ipcRenderer.invoke('pincer:secrets:set', input),
    setMany: (input) => ipcRenderer.invoke('pincer:secrets:setMany', input),
    delete: (name) => ipcRenderer.invoke('pincer:secrets:delete', name),
  },
  gatewayAdmin: {
  ...
  configuration: {
    ...
    authDetect: (agentId) => ipcRenderer.invoke('pincer:configuration:auth-detect', agentId),
    authStart: (input) => ipcRenderer.invoke('pincer:configuration:auth-start', input),
    authNext: (input) => ipcRenderer.invoke('pincer:configuration:auth-next', input),
    authCancel: (sessionId) => ipcRenderer.invoke('pincer:configuration:auth-cancel', sessionId),
    authLogout: (provider, profileIds, agentId) => ipcRenderer.invoke('pincer:configuration:auth-logout', provider, profileIds, agentId),
    authStatus: (refresh, agentId) => ipcRenderer.invoke('pincer:configuration:auth-status', refresh, agentId),
    modelsList: (agentId) => ipcRenderer.invoke('pincer:configuration:models-list', agentId),
    refreshProviderModels: (hash, id, apiKey) => ipcRenderer.invoke('pincer:configuration:provider-models-refresh', hash, id, apiKey),
    ...
    deleteProviderModel: (hash, id, modelId) => ipcRenderer.invoke('pincer:configuration:provider-model-delete', hash, id, modelId),
  ...
  management: {
    ...
    bindChannelAgent: (channel, account, agent) => ipcRenderer.invoke('pincer:management:channel-agent-bind', channel, account, agent),
    channelQrStart: (account) => ipcRenderer.invoke('pincer:management:channel-qr-start', account),
    channelQrWait: (account, qr) => ipcRenderer.invoke('pincer:management:channel-qr-wait', account, qr),
    saveChannel: (channel, account, values) => ipcRenderer.invoke('pincer:management:channel-save', channel, account, values),
    deleteChannel: (channel, account) => ipcRenderer.invoke('pincer:management:channel-delete', channel, account),
    integrations: () => ipcRenderer.invoke('pincer:management:integrations'),
    searchPlugins: (query) => ipcRenderer.invoke('pincer:management:plugin-search', query),
    inspectPlugin: (pluginId) => ipcRenderer.invoke('pincer:management:plugin-inspect', pluginId),
    refreshPlugins: () => ipcRenderer.invoke('pincer:management:plugin-refresh'),
    installIntegration: (input) => ipcRenderer.invoke('pincer:management:integration-install', input),
    setIntegrationEnabled: (pluginId, enabled, reviewToken) => ipcRenderer.invoke('pincer:management:integration-enable', pluginId, enabled, reviewToken),
    uninstallPlugin: (pluginId) => ipcRenderer.invoke('pincer:management:plugin-uninstall', pluginId),
  ...
  desktop: {
    ...
    openExternal: (url) => ipcRenderer.invoke('pincer:desktop:open-external', url),
  },
  chat: {
    snapshot: () => ipcRenderer.invoke('pincer:chat:snapshot'),
    refresh: () => ipcRenderer.invoke('pincer:chat:refresh'),
    refreshModels: () => ipcRenderer.invoke('pincer:chat:models-refresh'),
  ...
  window: {
    ...
    setLanguage: (language) => ipcRenderer.invoke('pincer:window:language', language),
