    ...
    return (
      ...
      >
        <SelectPrimitive.Trigger ref={ref} id={id} title={title} aria-label={ariaLabel} aria-describedby={ariaDescribedBy} data-testid={testId} data-value={controlled === EMPTY_VALUE ? '' : controlled} className={cn(
          ...
          'hover:bg-black/[0.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background data-[state=open]:bg-black/[0.06] disabled:cursor-default disabled:opacity-45',
        ...
        <SelectPrimitive.Portal>
          <SelectPrimitive.Content position="popper" sideOffset={5} collisionPadding={10} className="z-[10000] max-h-[min(320px,var(--radix-select-content-available-height))] min-w-[var(--radix-select-trigger-width)] max-w-[var(--radix-select-content-available-width)] overflow-hidden rounded-xl border border-black/10 bg-surface-modal p-1 text-foreground shadow-xl data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 dark:border-white/10">
            <SelectPrimitive.Viewport className="max-h-[310px] overflow-y-auto">
              {options.map((option) => <SelectPrimitive.Item key={option.value || EMPTY_VALUE} value={option.value || EMPTY_VALUE} disabled={option.disabled} className="relative flex min-h-8 cursor-default select-none items-center whitespace-normal break-words rounded-lg py-1.5 pl-2.5 pr-8 text-sm outline-none transition-colors data-[disabled]:opacity-40 data-[highlighted]:bg-primary/10 data-[state=checked]:text-primary">
