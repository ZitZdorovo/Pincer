...
import type { CronJob, CronJobCreateInput, CronSchedule, ScheduleType } from './cron-types';
...
function TaskDialog({ open, job, configuredChannels, onClose, onSave }: TaskDialogProps) {
  const hostApi = useCronHost();
  const currentAgentId = useAgentsStore((state) => state.currentAgentId);
  const { t } = useTranslation('cron');
  ...
  const [selectedAgentId, setSelectedAgentId] = useState(job?.agentId || currentAgentId);
  ...
  if (prevOpen !== open) {
    ...
    if (open) {
      ...
      setSelectedAgentId(job?.agentId || currentAgentId);
        ...
        <Card data-testid="cron-task-dialog">
          ...
          <CardContent className="space-y-6 pt-4 overflow-y-auto flex-1 p-6">
            ...
            <div className="flex items-center justify-between bg-transparent p-4 rounded-2xl shadow-sm border border-black/5 dark:border-white/5">
              <div>
                <Label htmlFor="cron-enable-immediately" className="text-sm text-foreground/80 font-bold">{t('dialog.enableImmediately')}</Label>
                <p className="text-meta text-muted-foreground mt-0.5">{t('dialog.enableImmediatelyDesc')}</p>
              </div>
              <Switch id="cron-enable-immediately" checked={enabled} onCheckedChange={setEnabled} />
            </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        >
          <Switch
            ...
            aria-label={`${job.enabled ? t('stats.active') : t('stats.paused')}: ${job.name}`}
          />
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-end mt-2 pl-[62px] min-w-0">
        ...
        {/* Actions */}
        <div className="mt-auto flex justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
          <RemoteReadout label={t('settings:pincer.runHistory')} request={() => window.pincer.management.jobRuns(job.id)} />
          <Button
            variant="ghost"
            size="sm"
...
export function Cron({ workspace, connected }: { workspace: WorkspaceState | null; connected: boolean }) { return <CronDataProvider workspace={workspace} connected={connected}><CronView /></CronDataProvider>; }
export default Cron;
