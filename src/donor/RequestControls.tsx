...
export function RequestStats({ state }: { state: WorkspaceState | null }) {
  ...
  return <div ref={root} className="relative z-40 shrink-0"><button type="button" data-testid="chat-request-stats-button" onClick={() => setOpen(!open)} title={t('composer.requestStats')} aria-label={t('composer.requestStats')} aria-expanded={open} className="flex h-6 items-center justify-center gap-0.5 rounded-lg border border-border/70 bg-surface-modal px-1.5 text-muted-foreground transition-[transform,background-color,color] hover:bg-black/5 hover:text-foreground active:scale-95 dark:hover:bg-white/10"><span className="flex h-[18px] w-[18px] items-center justify-center rounded-full" style={{ background: `conic-gradient(currentColor ${percent ?? 0}%, hsl(var(--muted)) ${percent ?? 0}% 100%)` }}><span className="h-3 w-3 rounded-full bg-surface-sidebar" /></span>{percent !== undefined && <span className="font-mono text-2xs tabular-nums">{percent}%</span>}</button>
    ...
    <div className="mt-4 border-t border-border pt-3"><QuotaList key={state?.scope} scope={state?.scope || ''} compact model={state?.model || ''} provider={model?.provider} modelLabel={model?.name} /></div>
    </div>}
  </div>;
}
