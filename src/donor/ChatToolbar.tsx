  ...
  return (
    <div className="flex items-center gap-2">
      <div ref={root} className="relative hidden sm:block">
        <button type="button" data-testid="chat-header-agent" aria-expanded={open} aria-label={ru ? `Действующий агент: ${label}` : `Active agent: ${label}`} disabled={disabled || agents.length < 2} onClick={() => setOpen((value) => !value)} className="chat-compact-control flex items-center gap-1.5 rounded-lg border border-black/10 bg-white/70 px-3 py-1.5 text-xs font-medium text-foreground/80 transition-colors hover:bg-black/5 disabled:cursor-default disabled:bg-black/[.035] disabled:text-muted-foreground disabled:opacity-65 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 dark:disabled:bg-white/[.025]">
          <Bot className="h-3.5 w-3.5 text-primary" /><span className="chat-compact-label">{label}</span>{agents.length > 1 && <ChevronDown className="chat-compact-chevron h-3 w-3 text-muted-foreground" />}
        </button>
