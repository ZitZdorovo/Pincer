...
export function useProviderData(connected: boolean) {
  ...
  const accounts = useMemo<ProviderAccount[]>(() => (snapshot?.providers || []).map((provider) => ({ id: provider.id, vendorId: PROVIDER_TYPE_INFO.some((type) => type.id === provider.id) ? provider.id as ProviderType : 'custom', label: provider.id, authMode: provider.api === 'ollama' ? 'local' : 'api_key', baseUrl: provider.baseUrl, apiProtocol: provider.api as ProviderAccount['apiProtocol'], model: provider.models[0], metadata: { customModels: provider.models }, enabled: true, isDefault: false, createdAt: '', updatedAt: '' })), [snapshot]);
  ...
  return { accounts, statuses, vendors, defaultAccountId: null as string | null, loading, error, refreshProviderSnapshot,
    ...
    removeAccount: async (_id: string) => { throw new Error('Provider removal is not supported by this Gateway adapter yet.'); },
