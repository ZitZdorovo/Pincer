...
export function Approvals({ updateBusy, inline = false }: { updateBusy: boolean; inline?: boolean }) {
  ...
  const content = <><Button size="sm" variant="ghost" disabled={!state.connected || refreshing} onClick={() => { setRefreshing(true); setError(false); void window.pincer.approvals.refresh().then((result) => setError(!result.ok)).catch(() => setError(true)).finally(() => setRefreshing(false)); }}><RefreshCw size={14} className={`mr-2 ${refreshing ? 'animate-spin' : ''}`} />{refreshing ? (ru ? 'Обновляем…' : 'Refreshing…') : (ru ? 'Обновить' : 'Refresh')}</Button>
