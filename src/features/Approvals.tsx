...
export function Approvals({ updateBusy, inline = false }: { updateBusy: boolean; inline?: boolean }) {
  ...
  if (inline) return <section><h2 className="openx-section-title !mb-2">{ru ? 'Одобрения Gateway' : 'Gateway approvals'}</h2><p className="mb-4 text-sm text-muted-foreground">{ru ? 'Ожидающие запросы и недавние решения. Решение всегда принимает пользователь.' : 'Pending requests and recent decisions. The user always makes the decision.'}</p>{content}</section>;
  return <>
    <button disabled={updateBusy} onClick={() => setOpen(true)} className="fixed bottom-4 right-4 z-40 flex items-center gap-2 rounded-full border border-amber-500/30 bg-surface-modal px-4 py-2 text-xs shadow-lg" aria-label={ru ? 'Разрешения' : 'Approvals'}><ShieldQuestion size={15} className="text-amber-600 dark:text-amber-400" />{ru ? 'Разрешения' : 'Approvals'}{pending > 0 && <span className="rounded-full bg-amber-500/15 px-1.5 py-0.5 font-medium" aria-live="polite">{pending}</span>}</button>
