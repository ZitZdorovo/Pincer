...
export function Memory({ state, language, connected, onDirty, embedded = false }: { state: WorkspaceState | null; language: Language; connected: boolean; onDirty(value: boolean): void; embedded?: boolean }) {
  ...
  return <div className={embedded ? 'space-y-5' : 'mx-auto max-w-3xl space-y-5 p-8'} data-testid="memory-page">
    ...
    <div className="settings-card space-y-4">
      <div className="flex items-center gap-2"><span className="shrink-0 text-sm font-medium text-muted-foreground">{t('agent')}:</span><Select className="min-w-0 flex-1" aria-label={t('agent')} value={selected} disabled={dirty || busy || !connected} onChange={(event) => setAgent(event.target.value)}>{state?.agents.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
      <div className="settings-responsive-grid-3"><MemoryConfiguration connected={connected && !busy} saved={() => { setHealth(null); setNotice(language === 'ru' ? 'Настройки сохранены. После переподключения проверьте семантический поиск.' : 'Settings saved. Check semantic search after reconnecting.'); }} /><Button variant="outline" disabled={!connected || !selected || busy} onClick={() => { if (dirty) setConfirmLoad(true); else void load(); }}>{t('read')}</Button><Button variant="outline" disabled={!connected || !selected || busy} onClick={() => void run(async () => { const result = await window.pincer.memory.status(selected, true); if (result.ok) setHealth(result.value); else setError(result.error.message); })}>{t('probe')}</Button></div>
      <form onSubmit={(event) => { event.preventDefault(); void run(async () => { const result = await window.pincer.memory.search(selected, query); if (result.ok) setSearch(result.value); else setError(result.error.message); }); }} className="grid grid-cols-2 gap-2"><Input className="min-w-0 w-full" aria-label={t('query')} placeholder={t('query')} value={query} onChange={(event) => setQuery(event.target.value)} maxLength={4000} /><Button className="w-full" disabled={!connected || !selected || busy || !query.trim()}>{t('search')}</Button></form>
    </div>
    ...
    <ConfirmDialog open={confirmLoad} title={language === 'ru' ? 'Отбросить изменения?' : 'Discard changes?'} message={language === 'ru' ? 'Несохранённые изменения MEMORY.md будут потеряны.' : 'Unsaved MEMORY.md changes will be lost.'} confirmLabel={language === 'ru' ? 'Отбросить' : 'Discard'} cancelLabel={language === 'ru' ? 'Продолжить редактирование' : 'Keep editing'} variant="destructive" onCancel={() => setConfirmLoad(false)} onConfirm={() => { setConfirmLoad(false); void load(); }} />
  </div>;
}
