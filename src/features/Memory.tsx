...
export function Memory({ state, language, connected, onDirty, embedded = false }: { state: WorkspaceState | null; language: Language; connected: boolean; onDirty(value: boolean): void; embedded?: boolean }) {
  ...
  return <div className={embedded ? 'space-y-5' : 'mx-auto max-w-3xl space-y-5 p-8'} data-testid="memory-page">
    ...
    <div className="settings-card space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><MemoryConfiguration connected={connected && !busy} saved={() => { setHealth(null); setNotice(language === 'ru' ? 'Настройки сохранены. После переподключения проверьте семантический поиск.' : 'Settings saved. Check semantic search after reconnecting.'); }} />
      <Select aria-label={t('agent')} value={selected} disabled={dirty || busy || !connected} onChange={(event) => setAgent(event.target.value)}>{state?.agents.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
