...
export function Memory({ state, language, connected, onDirty, embedded = false }: { state: WorkspaceState | null; language: Language; connected: boolean; onDirty(value: boolean): void; embedded?: boolean }) {
  ...
  return <div className={embedded ? 'space-y-5' : 'mx-auto max-w-3xl space-y-5 p-8'} data-testid="memory-page">
    ...
    {search && <section className="rounded-xl border border-border p-4"><p className="mb-2 text-xs text-muted-foreground">{search.semantic === false ? t('keyword') : search.semantic === null ? t('verifySearch') : t('ready')}</p><pre className="whitespace-pre-wrap break-words text-xs leading-6">{search.text}</pre></section>}
