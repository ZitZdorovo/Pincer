...
export function Files({ sessionKey, close, onDirty }: { sessionKey: string; close(): void; onDirty(value: boolean): void }) {
 const { t } = useTranslation('chat'); const ru = usePreferences().language === 'ru';
 ...
 const [width, setWidth] = useState(45); const panel = useRef<HTMLElement>(null);
 const resize = useRef<{ x: number; width: number; parent: number } | null>(null); const generation = useRef(0);
 ...
 return <aside ref={panel} data-testid="workspace-files" className="relative h-full min-h-0 shrink-0 border-l border-border bg-background" style={{ width: width + '%' }}>
  <div role="separator" aria-label={ru ? 'Ширина панели файлов' : 'File panel width'} aria-orientation="vertical" tabIndex={0} className="absolute inset-y-0 -left-1 z-40 w-2 cursor-col-resize" onPointerDown={(event) => { resize.current = { x: event.clientX, width, parent: panel.current?.parentElement?.clientWidth || 1 }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (resize.current) setWidth(Math.max(28, Math.min(75, resize.current.width + (resize.current.x - event.clientX) / resize.current.parent * 100))); }} onPointerUp={() => { resize.current = null; }} onPointerCancel={() => { resize.current = null; }} onKeyDown={(event) => { if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') { event.preventDefault(); setWidth((value) => Math.max(28, Math.min(75, value + (event.key === 'ArrowLeft' ? 1 : -1)))); } }} />
