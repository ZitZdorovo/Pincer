...
export function AppearanceExtras() {
  ...
  return <div className="divide-y divide-border rounded-2xl border border-border bg-surface-modal px-5" id="settings-typography">
    ...
    ] as const).map(([id, text, color]) => <button key={id} aria-label={text} title={text} aria-pressed={p.accentColor === id} onClick={() => setPreferences({ accentColor: id })} className={cn('h-6 w-6 rounded-full border border-surface-modal ring-offset-1 ring-offset-surface-modal', color, p.accentColor === id && 'ring-1 ring-foreground')} />)}</div></SettingRow>
