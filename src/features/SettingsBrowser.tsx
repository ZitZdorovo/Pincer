...
function PrimitiveControl({ schema, value, set, path, hint }: { schema: JsonSchema; value?: JsonValue; set(value: JsonValue | undefined): void; path: string[]; hint: SettingHint }) {
  ...
  if (enumValues.length > 0 && enumValues.length <= 5 && enumValues.every((item) => ['string', 'number', 'boolean'].includes(typeof item)) && enumValues.reduce<number>((length, item) => length + (ru ? russianEnumValue(item) : String(item)).length, 0) <= 32) {
  ...
  if (enumValues.length > 0) {
    return <Select aria-label={label} value={value === undefined ? '' : JSON.stringify(value)} onChange={(event) => set(event.target.value ? JSON.parse(event.target.value) as JsonValue : undefined)}><option value="">{ru ? 'По умолчанию' : 'Default'}</option>{enumValues.map((item) => <option key={JSON.stringify(item)} value={JSON.stringify(item)}>{ru ? russianEnumValue(item) : String(item)}</option>)}</Select>;
  }
...
function SettingField({ schema: rawSchema, value, set, path, hints, depth = 0, query = '' }: { schema: JsonSchema; value?: JsonValue; set(value: JsonValue | undefined): void; path: string[]; hints: Record<string, SettingHint>; depth?: number; query?: string }) {
  ...
  const developerMode = usePreferences().devMode;
  ...
  if (object) {
    ...
    return <details className="oc-setting-group" open={query ? true : depth <= 1} data-setting-path={fieldPath}>
      ...
      <div className="oc-setting-group-body">
        {!!variants?.length && <div className="oc-setting-row"><span className="text-sm font-medium">{ru ? 'Формат настройки' : 'Setting format'}</span><div className="oc-setting-control"><Select aria-label={`${label} ${ru ? 'тип' : 'type'}`} value={variant ?? ''} onChange={(event) => { const next = Number(event.target.value); setVariant(next); set(seed(variants[next])); }}><option value="" disabled>{ru ? 'Выбрать формат' : 'Choose format'}</option>{variants.map((item, index) => <option key={index} value={index}>{ru ? russianEnumValue(item.type || item.title || index + 1) : item.title || String(item.type || index + 1)}</option>)}</Select></div></div>}
